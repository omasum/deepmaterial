...
from deepmaterial.losses.losses import g_path_regularize, r1_penalty
from deepmaterial.utils import imwrite, tensor2img, Render, preprocess
from deepmaterial.utils.registry import MODEL_REGISTRY
from deepmaterial.utils.backups.render_util import toHDR_torch, toLDR_torch
from .base_model import BaseModel



@MODEL_REGISTRY.register()
class JitterGAN(BaseModel):
    """StyleGAN2 model."""

    def __init__(self, opt):
        super(JitterGAN, self).__init__(opt)

        # define network net_g
        self.net_g = build_network(opt['network_g'])
        self.net_g = self.model_to_device(self.net_g)
        self.print_network(self.net_g)
        self.net_e = build_network(opt['network_e'])
        self.net_e = self.model_to_device(self.net_e)
        ...
        load_path = self.opt['path'].get('pretrain_network_e', None)
        if load_path is not None:
            param_key = self.opt['path'].get('param_key_e', 'params')
            self.load_network(self.net_g, load_path, self.opt['path'].get('strict_load_e', True), param_key)
        ...
        self.opt['brdf_args']['toLDR'] = False
        self.svBRDF_utils = Render(self.opt['brdf_args'])
    ...
    def init_training_settings(self):
        ...
        self.net_e_ema = build_network(self.opt['network_e']).to(self.device)
        # load pretrained model
        load_path = self.opt['path'].get('pretrain_network_g', None)
        if load_path is not None:
        ...
        load_path = self.opt['path'].get('pretrain_network_e', None)
        if load_path is not None:
            self.load_network(self.net_e_ema, load_path, self.opt['path'].get('strict_load_e', True), 'params_ema')
        else:
            self.model_ema(0)  # copy net_g weight
        ...
        # render loss
        self.pix_render = build_loss(train_opt['pix_opt']).to(self.device)
    ...
    def feed_data(self, data):
        self.real_brdf = data['svbrdfs'].to(self.device)
        current_iter = data['iter']
        if current_iter == 1:
            self.real_img = data['inputs'].to(self.device)
            self.light = data['light'].to(self.device)
            self.view = data['view'].to(self.device)
            self.light_dis = data['dis'].to(self.device)
    ...
    def optimize_parameters(self, current_iter):
        ...
        embedding = self.net_e(preprocess(self.real_img))
        fake_img, _ = self.net_g(noise, init=embedding[-1])
        fake_pred = self.net_d(fake_img.detach())

        real_pred = self.net_d(self.real_brdf)
        ...
        if current_iter % self.net_d_reg_every == 0:
            self.real_brdf.requires_grad = True
            real_pred = self.net_d(self.real_brdf)
            l_d_r1 = r1_penalty(real_pred, self.real_brdf)
        ...
        re_img, _, _, _, _ = self.svBRDF_utils.render(fake_img, light_dir=self.light, view_dir=self.view, light_dis=self.light_dis, n_xy=True)
        # re_img = toHDR_torch(toLDR_torch(img))
        if self.svBRDF_utils.nbRendering == 1:
            re_img.squeeze_(1)
        
        # wgan loss with softplus (non-saturating loss) for generator
        l_g = self.cri_gan(fake_pred, True, is_disc=False)
        l_g_pix = self.pix_render(self.real_img, re_img)
        loss_dict['l_g'] = l_g + l_g_pix
        ...
        if current_iter % self.net_g_reg_every == 0:
            ...
            fake_img, latents = self.net_g(noise, init=embedding[-1][:path_batch_size], return_latents=True)
