...
from deepmaterial.utils import imwrite, tensor2img
from deepmaterial.utils.registry import MODEL_REGISTRY
from deepmaterial.utils.backups.render_util import torch_norm
...
class StyleGAN2Model(BaseModel):
    ...
    def feed_data(self, data):
        self.real_img = data['normal'].to(self.device)
    ...
    def get_normal(self, x):
        b,c,h,w = x[0].shape
        z_mat = torch.ones((b,1,h,w),dtype=torch.float32).to(self.device)
        normal = torch_norm(torch.cat([x[0],z_mat],dim=1))
        return normal, x[1]
    ...
    def optimize_parameters(self, current_iter):
        ...
        if current_iter % self.net_g_reg_every == 0:
            ...
            # latents = self.net_g(noise, only_latents=True)
            ...
            l_g_path = (self.path_reg_weight * self.net_g_reg_every * l_g_path)
