...
class VW_testModel(SurfaceNetModel):

    def __init__(self, opt):
        super(VW_testModel, self).__init__(opt)     
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            output, filters = self.net_g(self.inputs)
            normal, diffuse,roughness,specular = torch.split(output,[2,3,1,3],dim=1)
    ...
    def validation(self, dataloader, current_iter, tb_logger, save_img=False):
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            if self.opt['val'].get('save_img', False):
                results = self.get_current_visuals(self.output.cpu(), self.svbrdf.cpu(), self.filters.cpu())
                ...
                if self.opt['is_train'] or self.opt['val'].get('save_gt', False):
                    ...
                    self.save_visuals(brdf_path, results['predsvbrdf'], results['gtsvbrdf'], results['filter'])
                else:
    ...
    def get_current_visuals(self, pred, gt, filter):
        ...
        out_dict['filter'] = filter.detach().cpu()
    ...
    def save_visuals(self, path, pred, gt, filter):
        output = torch.cat([gt, pred], dim = 2) * 0.5 + 0.5 # (0,1) (batchsize, 10, 256*2, 256)
        normal, diffuse, roughness, specular = torch.split(output,[3,3,1,3],dim=1)
        roughness = torch.tile(roughness,[1,3,1,1])
        output = torch.cat([normal,diffuse,roughness,specular],dim=-1) # (batchsize, 3, 256*2, 256*4)
