...
# add learned fourier filter to get 4 maps splitly, especially for test, output covers filter weights
import os
import torch
import numpy as np
...
class VGG_wfc_sf_test(nn.Module):
    ...
    def __init__(self, vgg_name='VGG19'):
        super(VGG_wfc_sf_test, self).__init__()
    ...
    def img_process(self,img,l_weight):
        ...
        filters = [] # four filters of different maps
        consolidate = []
        new_img = torch.ones_like(image) # new batchsize image
        
        for w in weight:
            filter = self.filter(image,w) # [batchsize,3,h,w]
            ...
            filters.append(filter) # [4tensor(batchsize,3,h,w)]
        
        result = torch.cat(consolidate,dim=1)
        filters = torch.cat(filters,dim=1)
        return result, filters  # result: filtered image[batchsize,12,h,w], filters: [batchsize,4*3,h,w]
