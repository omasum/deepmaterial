...
class NAFNetPick(nn.Module):
    ...
    def __init__(self, in_channel=10, out_channel=10, width=16, middle_blk_num=1, enc_blk_nums=[], dec_blk_nums=[], res=False, tanh=True, **kwargs):
        super().__init__()
    ...
    def forward(self, inp, pattern=None):
        ...
        inp = torch.cat([inp, self.masked_HF], dim=1) # [B, 10, H, W]
        inp = self.check_image_size(inp)
    ...
    def __init__(self, in_channel=10, out_channel=10, width=16, middle_blk_num=1, enc_blk_nums=[], dec_blk_nums=[], res=False, tanh=True, **kwargs):
        ...
        if self.usePattern:
            ...
            self.intro = nn.Conv2d(in_channels=in_channel, out_channels=width, kernel_size=3, padding=1, stride=1, groups=1,
                              bias=True)
        self.encoders, self.middle_blks, self.downs, width = self.build_encoders(width, enc_blk_nums, middle_blk_num)
        self.sups, self.sdecoders, self.padder_size, widths = self.build_decoders(width, dec_blk_nums)
        self.ups, self.decoders, self.padder_size, width = self.build_decoders(width, dec_blk_nums)
        self.sending = nn.Conv2d(in_channels=widths, out_channels=3, kernel_size=3, padding=1, stride=1, groups=1,
                              bias=True)
        self.ending = nn.Conv2d(in_channels=width, out_channels=7, kernel_size=3, padding=1, stride=1, groups=1,
...
@ARCH_REGISTRY.register()
class NAFNSDNet_initial(nn.Module):
...
class NAFNetAB(nn.Module):
    ...
    def __init__(self, in_channel=3, out_channel=10, width=16, middle_blk_num=1, enc_blk_nums=[], dec_blk_nums=[], res=False, tanh=True, **kwargs):
        ...
        if self.usePattern:
            ...
            self.intro = nn.Conv2d(in_channels=in_channel, out_channels=width, kernel_size=3, padding=1, stride=1, groups=1,
                              bias=True)
            self.intro2 = nn.Conv2d(in_channels=8, out_channels=width, kernel_size=3, padding=1, stride=1, groups=1,
                              bias=True)
        self.encoders, self.middle_blks, self.downs, original_width = self.build_encoders(width, enc_blk_nums, middle_blk_num)
        self.HFencoders, self.HFmiddle_blks, self.HFdowns, width = self.build_encoders(width, enc_blk_nums, middle_blk_num)
        width = width//2
    ...
    def forward(self, inp, pattern=None):
        ...
        input_bands = self.inputs_bands # [B, 8, H, W]
