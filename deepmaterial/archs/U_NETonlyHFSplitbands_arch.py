...
class U_NETonlyHFSplitbands(nn.Module):
    ...
    def __init__(self):
        super(U_NETonlyHFSplitbands, self).__init__()
        # self.unet = UNet(3,12)
        self.encoder = Encoder(7,layers=True)
    ...
    #     self.allchannel = torch.cat([self.HighFrequency, self.Rchannel, self.Bchannel], dim=1) # [B, 10, H, W]
        self.HighFrequency = torch.ones(b, c, h, w)
