...
class wfc_sf_test(nn.Module):
    ...
    def __init__(self, vgg_name='VGG19'):
        super(wfc_sf_test, self).__init__()
        self.unetn = UNet(3,2)
        self.unetd = UNet(3,3)
        self.unetr = UNet(3,1)
        self.unets = UNet(3,3)
    ...
    def forward(self, x):
        outn = self.unetn(x) # [batchsize,2,h,w]
        outd = self.unetd(x) # [batchsize,3,h,w]
        outr = self.unetr(x) # [batchsize,1,h,w]
        outs = self.unets(x) # [batchsize,3,h,w]
        out = torch.cat([outn,outd,outr,outs],dim=1) # [batchsize,9,h,w]
        filter = torch.ones(size=(x.shape[0], 12, x.shape[2], x.shape[3]), dtype= torch.float32)
